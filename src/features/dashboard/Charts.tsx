...
import type { DayPoint, StatusSlice } from "@/shared/api/settings";
import { Card, EmptyState } from "@/shared/ui";

/** Круговая (donut) диаграмма распределения заказов по статусам. */
...
/** Столбики выручки и прибыли по дням за 2 недели. */
export function RevenueBars({ days }: { days: DayPoint[] }) {
  ...
  const totalProfit = days.reduce((a, d) => a + d.profit, 0);
  const W = 320, H = 120, n = days.length, gap = 4;
  const slot = n > 0 ? (W - gap * (n - 1)) / n : W;
  const barW = slot / 2;

  return (
    <Card title="Выручка и прибыль за 14 дней">
      {days.every((d) => d.revenue === 0 && d.profit === 0) ? (
        <EmptyState text="За последние 14 дней выдач не было" />
      ) : (
        <>
          ...
          <div className="mt-3 flex flex-wrap gap-4 text-sm">
            <span className="flex items-center gap-2">
              <span className="h-3 w-3 rounded-sm bg-success" /> Выручка: <b>{formatMoney(totalRev)}</b>
            </span>
            <span className="flex items-center gap-2">
              <span className="h-3 w-3 rounded-sm bg-primary" /> Прибыль: <b>{formatMoney(totalProfit)}</b>
            </span>
          </div>
        </>
      )}
