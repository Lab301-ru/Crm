-- ============================================================
-- Защита от «фантомной» выручки: заказы с датой выдачи в БУДУЩЕМ
-- (issued_at вперёд) не учитываются в выручке/прибыли ни за один период.
-- Считаем только фактически выданное не позже текущего момента.
-- ============================================================
...
begin
  ...
  with issued as (
    ...
    where t.deleted_at is null and t.status = 'issued'
      and t.issued_at is not null and t.issued_at <= now()
  ),
  agg as (
    select
